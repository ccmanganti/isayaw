        ...
        <div class="content">
            <div class="assessment-form">
                <form action="/assess" method="POST" class="form-assess">
                    ...
                    <input type="hidden" name="moduleid" readonly value="Module 2">
                    
                    @foreach (\App\Models\Assessment::select('questions')->where('module', 'Module 2')->get() as $question)
                        @foreach ($question['questions'] as $q)
                            <div class="questions">
                                <div class="question">
                                    <label for={{ $loop->index }} class="question-img">{!! $q['question'] !!}</label>
                                </div>
                ...
                @if ((\App\Models\Assessment::select('questions')->where('module', 'Module 2')->first()) == null)
